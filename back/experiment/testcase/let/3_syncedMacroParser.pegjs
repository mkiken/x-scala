...
/* Initializer written by homizu */
{
  ...
  //引数をフィルターして適切な形に変形する
	...
	function isVariable(e){
		//for version 0.8
		return (typeof e == 'object' && typeof e.type != 'undefined' && e.type == "Variable");
	}
...
MetaVariableDeclaration
  = type:( "Identifier" / "Expression" / "Type" / "Symbol") ___ list:VariableList ___ ";" {
        metaVariables[type] = metaVariables[type].concat(list);
    }
  / "Keyword" ___ list:LiteralKeywordList ___ ";" {
...
// リテラルキーワード "=>" は禁止
LiteralKeyword
	= !MacroArrow key:(l:Literal{return l.value;} / IdentifierName ) {return key;}
...
// パターン
MacroPattern
	= (UNDER / !MacroArrow IdentifierName) ___ patterns:SubPatternList? { return patterns || []; }
...
Statement
	= &{return macroType == "Expression"} e:Expr {return e;}
	/ &{return macroType == "Type"} t:Type {return t;}
...
plainid = start:upper parts:idrest {return start + parts;}
		...
		/ op
id
= &(&{return cExpression == 0} / !RejectWords)
	!KEYWORDS
name:(
...
InfixExpr = head:PrefixExpr tails1:(id nl? InfixExpr)* el:ExprEllipsis? tails2:InfixExpr?
{
	var seq = [head];
	var len1 = isNull(tails1)? -1 : tails1.length - 1;
	...
	if(!isNull(el)){
		...
		if(len1 >= 0) seq.push([tails1[len1][0], tails1[len1][2]]);
		seq.push(el);
	}
	else if(len1 >= 0){
		seq.push(tails1[len1][0]);
		seq.push(tails1[len1][2]);
	}
	if(!isNull(tails2)) seq.push(tails2);
	return {type:"InfixExpression", seq:seq};
}
...
/ &{return cTemplate > 0} OPVIR exp:Exprs? CLVIR se1:_SimpleExpr1 {return {type:"VirtualExpression", expr:exp, suffix:se1}; }
/ NEW arg:(ClassTemplate / TemplateBody) DOT id:id se1:_SimpleExpr1 {return {type:"InstanceCreationExpressionWithId", arg:arg, id:id, suffix:se1}; }
/ NEW arg:(ClassTemplate / TemplateBody) ta:TypeArgs se1:_SimpleExpr1 {return {type:"InstanceCreationExpressionWithTypes", arg:arg, types:ta, suffix:se1}; }
...
Generator = pt1:Pattern1 LEFTARROW expr:Expr guard:Guard? {return {type: "Generator", pt1:pt1, expr:expr, guard:ftr(guard)}; }
...
/ OPPAREN tps:Types CLPAREN  tails:(TypeArgs / withId)* {return {type:"SimpleTypes", types:tps, postfix:tails}; }
...
PatDef = ptn:Pattern2 ptns:(COMMA Pattern2)* tp:(COLON Type)? EQUAL expr:Expr{
	...
	for (var i = 0; i < result.length; i++) {
		...
		if(e.type == "InfixOperatorPattern"){
			if(e.ids.length){
				throw new Error("scala-parser.PatDef : ids exist. => " + e.type);
			}
			if(!isVariable(e.simplePattern)){
				throw new Error("scala-parser.PatDef : pattern is not Variable type. => " + e.type);
			}
			patterns.push({type: "PatDefVariable", id: e.simplePattern.name, tp: ttype});
		}
		else{
			throw new Error("scala-parser.PatDef : type is not infix operator pattern. => " + e.type);
		}
	}
...
/ id:ids COLON tp:Type EQUAL ud:UNDER {
	...
	for (var i = 0; i < id.ids.length; i++) {
		...
		if(e.type == "Variable" || e.type == "Variable2"){
			...
			throw new Error("scala-parser.VarDef : type is not infix operator pattern. => " + e.type);
		}
	}
...
/ v:varid {return {type: "Variable", name: v}}
/ StableId
...
OPVIR = '[#' ___ {return {type:"Keyword", word:"[#"}}
CLVIR = '#]' __ {return {type:"Keyword", word:"#]"}}
