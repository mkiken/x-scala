...
var trans = {
	...
	do_let : function(e, pos){
	    console.error("do_let :: pos = %d\ne = %j", pos, e);
	},
	...
	do_letrec_star : function(e, pos){
        ...
        for(var i = pos+1; i < e.length; i++){
            ...
            //BlockStatの解析
            // if(!this.isNull(stat)){
            if(Array.isArray(stat) && stat[0] === 'Scala'){
			    trans.s2j(stat, 0);
			    B.format(';', newline);
            }
        }
	...
	do_scala : function(e, pos){
		...
		if(type == "AnonymousFunction"){
			...
			if(!this.isNull(e[pos+1])) this.s2j(e[pos+1], 0);
			...
			if(!this.isNull(e[pos+1])) this.s2j(e, pos + 1);
	...
	do_list : function(es, pos){
		...
		if(typeof(e) == "string"){
			...
			else if(e.name == "let") this.do_letrec_star(es, pos+1);
