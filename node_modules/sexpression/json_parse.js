...
// シンプルな再帰下降パーサで実装されています
//
// 実行例:
// var obj = json_parse("{\"first\": \"Jerome\"}");
//
// 実行例(全ての値に操作を行うreviver関数ありの場合):
// var obj =  json_parse("{\"first\": \"Jerome\"}",
// function(key, value){if(key==='first') return 'hoge'; return value;});
//
// Chromeの方は、ツール > JavaScriptコンソールに、このソースを全て貼り付けて、
// 実行例を実行して、すぐに動作確認できます。


// グローバル変数を生成することを避けるため、以下の変数及び関数は、
// 関数の中で定義されます
...
// 一文字次に進める関数
// もしパラメーターcが指定されていたら、
// それが現在の文字にマッチするのかを調べ、矛盾がありそうならエラーで終わらせます
...
// 配列を解析する関数
             ...
             if (ch === '[') {
                 ...
                 while (ch) { //値があるなら、value関数の実行結果を配列に入れる
                     array.push(value());
                     white();
                     if (ch === ']') {
                         next(']');
